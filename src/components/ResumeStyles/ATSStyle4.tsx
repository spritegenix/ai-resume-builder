...
export default function ATSStyle4({
  ...
  const [adjustedColorHex, setAdjustedColorHex] = useState(
    hexToRgbWithAlpha("#7BDCB5", 20),
  );
  const { width } = useDimensions(containerRef);

  useEffect(() => {
    const newColor = hexToRgbWithAlpha(resumeData?.colorHex || "#7BDCB5", 20);
    setAdjustedColorHex(newColor);
  }, [resumeData.colorHex]);
      ...
      >
        <div className="grid grid-cols-2">
          <div
            ...
            style={{
              backgroundColor: adjustedColorHex,
            }}
          >
            {" "}
            {resumeData.photo ? (
            ...
            {!!resumeData.skills && resumeData.skills?.length > 0 && (
              <>
                ...
                {resumeData.skills?.map((skill, index) => (
                  ...
                  >
                    <div className="flec-col !m-0 flex items-center justify-between">
                      <div>
              ...
              <>
                ...
                {resumeData.educations?.map((edu, index) => (
                  <div key={index} className="!m-0 break-inside-avoid">
                    ...
                    <p className="!m-0 flex w-full flex-col justify-between">
                      <span>Percentage - {edu.marks}</span>
                    </p>
                    <p>{edu.description}</p>
                  </div>
                ))}
            ...
            {!!resumeData.certifications &&
              resumeData.certifications?.length > 0 && (
                <>
                  ...
                  {/* <div
                    className={`flex flex-wrap ${resumeData.certifications.find((skill) => skill.description) && "flex-col"}`}
                  > */}
              ...
              resumeData.projectWorks?.length > 0 && (
                <>
                  ...
                  {resumeData.projectWorks?.map((item, index) => (
                    <div
                      key={index}
                      className="!m-0 break-inside-avoid space-y-1 pb-3"
                    >
                      <div className="!m-0 flex flex-col justify-between gap-1">
...
function PersonalInfoHeader1({ resumeData }: { resumeData: ResumeValues }) {
  ...
  return (
    <div className="mb-2">
      <Link href={resumeData.portfolioLink || "#"} className="cursor-pointer">
        <p
          className="text-[3em] font-bold"
          style={{
            color: colorHex,
      ...
      {/* Social Links  */}
      <div className="mx-auto max-w-xl gap-x-4">
        {(city || country) && (
          <p className="flex items-center gap-1">
            <BiSolidMap />
            {city}
  ...
  return (
    <>
      <div className="flex break-inside-avoid gap-x-0.5">
        <h1
          className="flex flex-col text-nowrap text-[1.2em] font-semibold"
          style={{
            color: colorHex,
          }}
        >
...
const hexToRgbWithAlpha = (hex: string, alphaPercent: number) => {
